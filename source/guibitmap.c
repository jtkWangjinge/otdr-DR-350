...
int LoadBitmapImage(GUIBITMAP *pBitmapObj)
{
    ...
    {
        //尝试读取位图文件头和信息头
        if (-1 == lseek(pBitmapObj->iBitmapFd, 0, SEEK_SET))
        {
            iErr = -4;
        }
  ...
  #error "Unknown GUI_OS_ENV, not supported current GUI_OS_ENV!"
    ...
    if (!iErr)
    {
        ...
        if (-1 == lseek(pBitmapObj->iBitmapFd, fh.uiDataOffset, SEEK_SET))
        {
            iErr = -9;
        }
        ...
        {
            if (-1 == read(pBitmapObj->iBitmapFd, pBuf, uiBmp)) //读入一行
            {
                iErr = -9;
                break;
            }
            memcpy(pRes, pBuf, uiImg);                          //写入该行
            pRes -= uiImg;
        }
        //释放行缓冲
        free(pBuf);
...
GUIPALETTE* ReadBitmapPalette(GUIBITMAP *pBitmapObj)
{
  ...
  #error "Unknown GUI_OS_ENV, not supported current GUI_OS_ENV!"
    ...
    {
        //尝试读取位图中的调色板
        if (-1 == lseek(pBitmapObj->iBitmapFd, 54, SEEK_SET))
        {
            iErr = -8;
        }
