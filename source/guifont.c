                    ...
                    unsigned int uiFgColor, unsigned int uiBgColor)
{
    ...
    if (!iErr)
    {
        ...
        //判断字体的宽度和高度是否有效
        if (0 == uiFontWidth || 0 == uiFontHeight)
        {
            iErr = -1;
        }
    }
    ...
    {
        //尝试读取字体文件头
        if (-1 == lseek(pFontObj->iFontFd, 0, SEEK_SET))
        {
            iErr = -5;
        }
...
/***
  * 功能：
        从指定字体对象中提取特定编码的字体点阵
  * 参数：
        1.unsigned short *usMatrixCode:  字符编码指针
        2.GUIFONT *pFntObj:             需要从中提取字体点阵的字体对象
        3.int len:						长度
        4.int isRight:				向右连接还是向左连接,0--连到左边字符上，1--连到右边字符上
  * 返回：
        成功返回有效指针，失败返回NULL
  * 备注：
***/
                                  ...
                                  GUIFONT *pFontObj, int len, int isRight)
{
  ...
  #error "Unknown GUI_OS_ENV, not supported current GUI_OS_ENV!"
    ...
    if (!iErr)
    {
    	//阿拉伯语音标，读出每一个字体编码，然后进行或操作
        {
        	//先读出当前的
        	if (-1 == lseek(pFontObj->iFontFd, fh.uiDataOffset + (*usMatrixCode) * uiSize, SEEK_SET))
	        {
	            iErr = -6;
	        }
		
        	if (-1 == read(pFontObj->iFontFd, pReturn->pMatrixData, uiSize))
	        {
	            iErr = -6;
	        }
			...
			{
				int tmp = isRight ? -i : i;
				unsigned char *tmpdata = (unsigned char *) calloc(uiSize,1);
				if (NULL == tmpdata)
		        {
		            iErr = -6;
		        }
				//读取文件中的点阵数据至字体点阵
		        if (-1 == lseek(pFontObj->iFontFd, fh.uiDataOffset + (*(usMatrixCode + tmp)) * uiSize, SEEK_SET))
		        {
		            iErr = -6;
		        }

	        	if (-1 == read(pFontObj->iFontFd, tmpdata, uiSize))
		        {
		            iErr = -6;
		        }
				...
				if (tmpdata)
				{
					...
					for (j = 0; j < uiSize; j++)
					{
						pReturn->pMatrixData[j] |= tmpdata[j];
					}
					free(tmpdata);
					tmpdata = NULL;
				}
  ...
  #error "Unknown GUI_OS_ENV, not supported current GUI_OS_ENV!"
    ...
    if (!iErr)
    {
        ...
        {
        	// 特殊语言，而且不需要清除左边空白
        	if (isArabic() && !isClearLeft(*usMatrixCode))
        	{
				uiLeft = 0;
        	}
			else
        ...
        else
        {
        	...
        	if (isArabic())
        	{
				pReturn->uiRealWidth += getGap(*usMatrixCode);
        	}
        }
        //释放行缓冲
        free(pBuf);
                                  ...
                                  GUIFONT *pFontObj)
{
  ...
  #error "Unknown GUI_OS_ENV, not supported current GUI_OS_ENV!"
    ...
    {
        //读取文件中的点阵数据至字体点阵
        if (-1 == lseek(pFontObj->iFontFd, 
                        fh.uiDataOffset + usMatrixCode * uiSize,
                        SEEK_SET))
        {
            iErr = -6;
        }
